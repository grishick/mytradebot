    ...
    describe("heartbeat", function() {
        ...
        it("should 1. check for current order (find none), 2.check balance (find USD), 3. get advice, 4. place buy order", function(done) {
            ...
            mockTrader2.getAvailableBalance = function(mode, cb) {
                console.log("mockTrader :: getAvailableBalance")
                this.getAvailableBalanceCalled = true;
                cb(null, {"USD":2000, "BTC":0});
            }
            runner.heartbeat("live", "BTC-USD", 0.01, 0.005, 1000, 5000, "day", 2, mockTrader2, mockAdvisor, mockNotifier, function(err) {
                ...
                //assert.ok(mockTrader.getHighLowCalled, "should have called getHighLow");
        ...
        it("should 1. check for current order (find none), 2.check balance (find USD), 3. get advice, 4. place buy order", function(done) {
            ...
            runner.heartbeat("live", "BTC-USD", 0.01, 0.005, 1000, 5000, "day", 2, mockTrader2, mockAdvisor2, mockNotifier, function(err) {
                assert.ok(!err, "should not return an error")
                assert.ok(mockTrader2.getCurrentOrderCalled, "should have called getCurrentOrder");
                assert.ok(mockTrader2.getAvailableBalanceCalled, "should have called getAvailableBalance");
                //assert.ok(mockTrader.getHighLowCalled, "should have called getHighLow");
        ...
        it("should 1.check current order (find buy order), 2. get advice", function(done) {
            ...
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 1000, 5000, "day", 2, mockTrader2, mockAdvisor2, mockNotifier, function(err) {
                ...
                assert.ok(!mockTrader2.getLatestTradesCalled, "should NOT have called getLatestTrades");
                assert.ok(!mockTrader2.getAvailableBalanceCalled, "should NOT have called getAvailableBalance");
                //assert.ok(mockTrader2.getHighLowCalled, "should have called getHighLow");
                assert.ok(mockAdvisor2.adviseCalled, "should have called advise");
                assert.ok(!mockTrader2.placeBuyOrderCalled, "should NOT have called placeBuyOrder");
                assert.ok(!mockTrader2.placeSellOrderCalled, "should NOT have called placeSellOrder");
                assert.ok(!mockTrader.getBalancesCalled, "should NOT have called getBalances");
                done();
            })
        })
        ...
        it("should 1.check current order (find sell order). 2. Replace it with a lower one", function(done) {
            ...
            //mode, productId, lookBackSeconds, candleGranularity, margin, trader, advisor, cb
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 2500, 5000, "day", 2, mockTrader2, mockAdvisor2, mockNotifier, function(err) {
                assert.ok(!err, "should not return an error")
                assert.ok(mockTrader2.getCurrentOrderCalled, "should have called getCurrentOrder");
                assert.ok(mockTrader2.updateOrderCalled, "should have called updateOrder");
                assert.ok(!mockTrader2.getLatestTradesCalled, "should NOT have called getLatestTrades");
                ...
                assert.ok(!mockTrader2.getBalancesCalled, "should NOT have called getBalances");
        ...
        it("should 1.check current order (find none), 2. check balance (find BTC), 3. fetch last fill, 4. place sell order", function(done) {
            this.timeout(10000);
            var runner = require("../lib/runner.js");
            ...
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 1000, 5000, "day", 2, mockTrader3, mockAdvisor2, mockNotifier, function(err) {
                ...
                assert.ok(mockTrader3.getBalancesCalled, "should have called getBalances");
        ...
        it("should 1.check current order (find done 'buy' order), 2. check balance (find BTC), 3. fetch last fill, 4. place sell order", function(done) {
            this.timeout(10000);
            var runner = require("../lib/runner.js");
            var expectedSellPrice = utils.round(2400.01 * 1.005, 2);
            var mockTrader3 = mockTrader;
        ...
        it("should 1.check current order (find buy order), 2.fetch low/high, 3. get advice, 4. check balance, 5. update buy order with higher price", function(done) {
            this.timeout(30000);
            var runner = require("../lib/runner.js");
            ...
            //mode, productId, lookBackSeconds, candleGranularity, margin, trader, advisor, cb
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 1000, 5000, "day", 2, mockTrader4, mockAdvisor4, mockNotifier, function(err) {
                assert.ok(!err, "should not return an error " + JSON.stringify(err))
                assert.ok(mockTrader4.getCurrentOrderCalled, "should have called getCurrentOrder");
                assert.ok(!mockTrader4.getLatestTradesCalled, "should NOT have called getLatestTrades");
                ...
                assert.ok(!mockTrader4.getBalancesCalled, "should NOT have called getBalances");
        ...
        it("should 1.check current order (find sell order). 2. update sell order and find that it matched. 3.check balance (find USD), 4.fetch high/low, 5. get advice, 6. place buy order", function(done) {
            ...
            //mode, productId, lookBackSeconds, candleGranularity, margin, trader, advisor, cb
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 1000, 5000, "day", 2, mockTrader5, mockAdvisor, mockNotifier, function(err) {
                assert.ok(!err, "should not return an error")
                ...
                //assert.ok(mockTrader5.getHighLowCalled, "should have called getHighLow");
                assert.ok(!mockTrader5.getLatestTradesCalled, "should NOT have called getLatestTrades");
                assert.ok(mockAdvisor.adviseCalled, "should have called advise");
                assert.ok(mockNotifier.notifyCalled, "should have called notify");
                assert.ok(mockTrader5.placeBuyOrderCalled, "should have called placeBuyOrder");
                assert.ok(mockTrader5.getBalancesCalled, "should have called getBalances");
        ...
        it("should 1.check current order (find done 'buy' order), 2. check balance (find BTC), 3. fetch last fill, 4. call getCurrentPrice, 5. place sell order for current price + 5c", function(done) {
            this.timeout(10000);
            var runner = require("../lib/runner.js");
            var expectedSellPrice = 2511.05;
            var mockTrader3 = mockTrader;
            mockTrader3.getAvailableBalance = function(mode, cb) {
                console.log("mockTrader3 :: getAvailableBalance")
                this.getAvailableBalanceCalled = true;
            ...
            runner.heartbeat("live", "BTC-USD",  0.01, 0.005, 1000, 5000, "day", 2, mockTrader3, mockAdvisor2, mockNotifier, function(err) {
                ...
                assert.ok(mockAdvisor2.adviseCalled, "should have called advise");
                assert.ok(!mockTrader3.placeBuyOrderCalled, "should NOT have called placeBuyOrder");
                assert.ok(mockTrader3.getCurrentPriceCalled, "should have called getCurrentPrice");
                assert.ok(mockNotifier.notifyCalled, "should have called notify");
                assert.ok(mockTrader3.placeSellOrderCalled, "should have called placeSellOrder");
                assert.ok(mockTrader3.getBalancesCalled, "should have called getBalances");
                var foundSellPrice = utils.round(mockTrader3.lastSellOrderPrice, 2);
