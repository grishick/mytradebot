    ...
    trader.getAvailableBalance(mode, function(err, balances) {
        if(err) {
            ...
            trader.getCurrentPrice(productId, function(err, currentPrice) {
                if(err) {
                    cb(err);
                } else if(balances[productId.substr(0,3)]) {
                    if(mode == "live") {
                        utils.sleep(1);
                    }
                                    ...
                                    if(err) {
                                        ...
                                        trader.getBalances(mode, function(err, allBalances) {
                                            if(err) {
                                                ...
                                                var msg = "Placed limit sell order at {time}. For {size} {productId} at {price}. Bought at {fill}. Balances: USD:{usd}, BTC: {btc}, BCH: {bch}, ETH: {eth}, LTC: {ltc}".format(
                                                    {
                                                        time: new Date().toLocaleString("en-us", {timeZone: "America/Los_Angeles"}),
                                                        productId: productId,
                                    ...
                                    if(err) {
                                        ...
                                        trader.getBalances(mode, function(err, allBalances) {
                                            if(err) {
                                                ...
                                                var msg = "Placed buy order at {time} for {cancelBuyAfter}. For {size} {productId} at {price}. Balances: USD:{usd}, BTC: {btc}, BCH: {bch}, ETH: {eth}, LTC: {ltc}".format(
                                                    {
                                                        time: new Date().toLocaleString("en-us", {timeZone: "America/Los_Angeles"}),
                                                        productId:productId,
                                                        size:buyParams.size,
                                ...
                                if(err) {
                                    ...
                                    if (balances["USD"] && balances["USD"] >= minTrade) {
                                        ...
                                        cb({
                                            ...
                                            message: "Not enough balance to place an order. USD balance: {usd}. Minimum trade: {minTrade}. Max trade: {maxTrade}. Buy price: {price}.".format({
                                                usd: balances["USD"],
                                                minTrade: minTrade,
                                                maxTrade: maxTrade,
                                                price: advice.price
